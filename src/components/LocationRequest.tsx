  ...
  return (
    ...
    >
      ...
      <div className="location-search-area">
        <div className="location-divider"><span>또는 원하는 위치로 찾기</span></div>
        <form className="location-search-form" onSubmit={submitSearch}>
          ...
          <button
            ...
            type="submit"
            disabled={!query.trim() || isSearching || searchStatus === 'unconfigured'}
          >
            {isSearching ? '검색 중…' : '검색'}
          </button>
        </form>
        ...
        {searchStatus === 'empty' && (
          <p className="location-search-message" role="status">
            검색 결과가 없어요. 동네 이름이나 주소를 조금 더 자세히 입력해 주세요.
          </p>
        )}
        {searchStatus === 'error' && (
