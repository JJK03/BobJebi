...
} from "../../../shared/api/kakao";
import { useGeolocation } from "../model/useGeolocation";
import { useRestaurants } from "../model/useRestaurants";
import { MenuRecommendationPage as App } from "./MenuRecommendationPage";

vi.mock("../model/useGeolocation", () => ({
  useGeolocation: vi.fn(),
}));

vi.mock("../model/useRestaurants", () => ({
  useRestaurants: vi.fn(),
}));

vi.mock("../../../shared/api/kakao", async (importOriginal) => {
  const actual =
    await importOriginal<typeof import("../../../shared/api/kakao")>();
...
describe("App 사용자 흐름", () => {
  ...
  it("인천 스마트음식관광 탭으로 데이터 소스를 전환한다", async () => {
    ...
    expect(useRestaurants).toHaveBeenLastCalledWith(
      "incheon-smart-food",
      USER_POSITION,
      undefined,
    );
